...
import { categories, transaction_interval, transaction_type, transactions, users } from "../types";
...
export const TransactionsUpdate = () => {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<formData>({
    ...
    defaultValues: {
      ...
      date: data ? new Date(Date.parse(data.date)) : new Date(),
  ...
  const updateHandler = async (data: formData) => {
    try {
      const response = await service.put(`/transactions/${id}`, {
        transaction: data
      })
  ...
  return (
    <>
      <Form onSubmit={handleSubmit(updateHandler)} title="Transaction Registration">
